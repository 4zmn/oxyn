...
local function get_server_mapping()
  local ok, mappings = pcall(require, "mason-lspconfig.mappings")
  if ok and type(mappings.get_mason_map) == "function" then
...
local function resolve_mason_config(server_name)
  ...
  local server_mapping = get_server_mapping()
...
function M.setup(server_name, user_config)
  ...
  if not registry.is_installed(pkg_name) then
    if should_auto_install(server_name) then
      ...
      Log:debug(string.format("Installation in progress for [%s]", server_name))
      local ok, pkg = pcall(registry.get_package, registry, pkg_name)
      if not ok then
        Log:debug(string.format("[%s] not yet available in mason registry, skipping auto-install", pkg_name))
        return
      end
